#Syntax errors on lines 42 and 46
"""
...
def optimal_merge_pattern(files: list) -> float:
    ...
    while len(files) > 1; # syntax error, expected : instead of ;
        temp = 0
        # Consider two files with minimum cost to be merged
        for _ in range(2):
            min_index = files.index(min(files) #syntax error, missing closing bracket
